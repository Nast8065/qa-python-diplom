...
class TestUserRegistration:
    ...
    def test_create_unique_user_success(self, api_available):
        ...
        with allure.step("Отправить запрос на создание пользователя"):
            response = requests.post(f"{urls.BASE_URL}/auth/register", json=user_data)
        ...
        with allure.step("Удалить созданного пользователя"):
            ...
            delete_response = requests.delete(f"{urls.BASE_URL}/auth/user", headers=headers)
            assert delete_response.status_code == 202, f"Ожидался статус 202 при удалении"
