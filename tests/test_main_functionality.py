...
class TestMainFunctionality:
    ...
    def test_ingredient_modal_closing(self, browser):
        ...
        main_page = MainPage(browser)
        order_modal = OrderModal(browser)
    ...
    def test_ingredient_counter_increase(self, authenticated_user):
        with allure.step("Получить начальное значение счетчика ингредиента"):
            initial_counter = authenticated_user.get_ingredient_counter(0)
        ...
        with allure.step("Проверить увеличение счетчика"):
            new_counter = authenticated_user.get_ingredient_counter(0)
            assert new_counter > initial_counter, f"Счетчик ингредиента должен увеличиться. Было: {initial_counter}, стало: {new_counter}"
